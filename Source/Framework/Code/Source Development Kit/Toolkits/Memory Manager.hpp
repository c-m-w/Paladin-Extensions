...
inline BOOL __stdcall LoadDependencies( void *pImage, void *pGetProcAddress, void *pLoadLibrary )
{
		  ...
		  p = reinterpret_cast< IMAGE_IMPORT_DESCRIPTOR * >( std::uintptr_t( p ) + sizeof( IMAGE_IMPORT_DESCRIPTOR ) ) )
	{
		...
		{
			const auto szImport = reinterpret_cast< const char * >( ( pOriginal->u1.Ordinal & IMAGE_ORDINAL_FLAG ) > 0 ? reinterpret_cast< void * >( pOriginal->u1.Ordinal & 0xFFFF ) : reinterpret_cast< IMAGE_IMPORT_BY_NAME * >( std::uintptr_t( pImage ) + pOriginal->u1.AddressOfData )->Name );
...
template< typename T > struct _RTL_INVERTED_FUNCTION_TABLE_ENTRY
{
...
template< typename T > struct _RTL_INVERTED_FUNCTION_TABLE
{
	ULONG Count;
	ULONG MaxCount;
	ULONG Epoch;
