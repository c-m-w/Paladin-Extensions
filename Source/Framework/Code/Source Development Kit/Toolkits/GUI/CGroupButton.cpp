...
void CGroupButton::CreateDrawables( )
{
	...
	bool bOutline[ ] = { _Type == BUTTON_LEFT, _Type == BUTTON_RIGHT, _Type == BUTTON_RIGHT, _Type == BUTTON_LEFT };

	pOutline->SetDrawingType( D3D11_PRIMITIVE_TOPOLOGY_LINELIST );
	pOutline->RoundedRectangle( recLocation, bOutline, GetCurrentColor( COLOR_INDEX_PRIMARY ), ROUNDING );
	pFill->RoundedRectangle( recLocation, bOutline, GetCurrentColor( COLOR_INDEX_SECONDARY ), ROUNDING );
