...
template< typename _Context > union UHookCallback
{
	void ( __cdecl *fn )( void *pThis, _Context * );
	void *p;
};

template< typename _t > class temp_ptr
{
	_t *p;
public:
	temp_ptr( )
	...
	template< typename... _Construction > temp_ptr( _Construction ... _Constructions )
	{
