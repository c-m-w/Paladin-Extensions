<?php
// FROM HASH: 05a21582e1a9979b5bef8b6a0a6d7554
return array('macros' => array(), 'code' => function($__templater, array $__vars)
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Audio proxy log');
	$__finalCompiled .= '
...
', array(
		'action' => $__templater->fn('link', array('logs/audio-proxy', ), false),
		'class' => 'block',
	)) . '
		...
		if ($__templater->isTraversable($__vars['entries'])) {
			foreach ($__vars['entries'] AS $__vars['entry']) {
				...
				if ($__vars['entry']['file_size']) {
					$__compilerTemp2 .= '<li>' . 'Size' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->filter($__vars['entry']['file_size'], array(array('file_size', array()),), true) . '</li>';
				}
				$__compilerTemp1 .= $__templater->dataRow(array(
				), array(array(
					'href' => $__templater->fn('link', array('logs/audio-proxy', $__vars['entry'], ), false),
					'overlay' => 'true',
					'_type' => 'cell',
					'html' => '
