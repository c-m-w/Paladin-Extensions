<?php
// FROM HASH: 9eb1f8333511ad54c5d7374d0f6134ee
...
';
	...
	' . $__templater->button('', array(
			'href' => $__templater->fn('link', array('resource-manager/categories/delete', $__vars['category'], ), false),
			'icon' => 'delete',
			'overlay' => 'true',
		), '', array(
		)) . '
...
';
	...
	if (!$__templater->test($__vars['availableFields'], 'empty', array())) {
		...
		$__compilerTemp2 .= '
				...
				' . $__templater->formRow('
					' . $__templater->filter('None', array(array('parens', array()),), true) . ' <a href="' . $__templater->fn('link', array('resource-manager/fields', ), true) . '" target="_blank">' . 'Add field' . '</a>
				', array(
			'label' => 'Available fields',
		)) . '
			';
	...
	if (!$__templater->test($__vars['availablePrefixes'], 'empty', array())) {
		...
		$__compilerTemp4 .= $__templater->formCheckBoxRow(array(
			...
			'hint' => 'If selected, users will be required to select a prefix when creating or editing a extension. This will not be enforced for moderators.',
			'_type' => 'option',
		)), array(
		)) . '
		...
		$__compilerTemp4 .= '
				...
				' . $__templater->formRow('
					' . $__templater->filter('None', array(array('parens', array()),), true) . ' <a href="' . $__templater->fn('link', array('resource-manager/prefixes', ), true) . '" target="_blank">' . 'Add prefix' . '</a>
				', array(
			'label' => 'Available prefixes',
		)) . '
	...
	array(
		...
		'label' => 'Allowed extension types',
		'explain' => 'If no extension types are selected, this category will be used to maintain the category hierarchy only.',
	)) . '
		...
		'label' => '
					' . 'Always moderate extensions posted in this category' . '
				',
		'_type' => 'option',
	),
	array(
		...
		'label' => '
					' . 'Always moderate extension updates posted in this category' . '
				',
		'_type' => 'option',
	)), array(
	)) . '
		...
		'explain' => 'This will require users to provide at least this many tags when creating an extension.',
	)) . '
		...
		'explain' => 'If selected, whenever a extension in this category is created, a thread will be posted in this forum.',
	)) . '
...
', array(
		'action' => $__templater->fn('link', array('resource-manager/categories/save', $__vars['category'], ), false),
		'class' => 'block',
		'ajax' => 'true',
	));
