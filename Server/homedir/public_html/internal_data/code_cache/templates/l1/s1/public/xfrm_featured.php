...
return array('macros' => array(), 'code' => function($__templater, array $__vars)
{
	...
	if ($__vars['category']) {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Featured extensions' . ' - ' . $__templater->escape($__vars['category']['title']));
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageH1'] = $__templater->preEscaped('Featured extensions');
		$__finalCompiled .= '
	';
...
';
		...
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Featured extensions');
		$__finalCompiled .= '
';
	}
	$__finalCompiled .= '
...
<div class="block" data-xf-init="' . ($__vars['canInlineMod'] ? 'inline-mod' : '') . '" data-type="resource" data-href="' . $__templater->fn('link', array('inline-mod', ), true) . '">
	...
	if (!$__templater->test($__vars['resources'], 'empty', array())) {
		...
		$__finalCompiled .= '
				<div class="block-row">' . 'No extensions have been featured yet.' . '</div>
			';
	}
	$__finalCompiled .= '
		</div>
