<?php
// FROM HASH: 99510ed0b2ca86621c8b6a7d3cd69bd6
return array('macros' => array('list' => function($__templater, array $__arguments, array $__vars)
{
	...
	' . $__templater->form('
		...
		'hint' => 'If selected, users will only be able to view this department if they are explicitly granted permissions.',
		'_type' => 'option',
	)), array(
	)) . '
			</div>
	...
	', array(
		'action' => $__templater->fn('link', array($__vars['routeBase'] . '/save', $__vars['department'], ), false),
		'ajax' => 'true',
		'class' => 'block',
	)) . '
	...
	if ($__templater->isTraversable($__vars['userGroups'])) {
		foreach ($__vars['userGroups'] AS $__vars['userGroup']) {
			$__compilerTemp1 .= '
				...
				'href' => $__templater->fn('link', array($__vars['routeBase'] . '/edit', $__vars['department'], array('user_group_id' => $__vars['userGroup']['user_group_id'], ), ), false),
		...
		if ($__templater->isTraversable($__vars['users'])) {
			foreach ($__vars['users'] AS $__vars['user']) {
				$__compilerTemp3 .= '
					...
					'href' => $__templater->fn('link', array($__vars['routeBase'] . '/edit', $__vars['department'], array('user_id' => $__vars['user']['user_id'], ), ), false),
	...
	', array(
		'action' => $__templater->fn('link', array($__vars['routeBase'] . '/edit', $__vars['department'], ), false),
		'class' => 'block',
	)) . '
';
...
'edit' => function($__templater, array $__arguments, array $__vars)
{
	...
	', array(
		'action' => $__templater->fn('link', array($__vars['routeBase'] . '/save', $__vars['department'], $__vars['saveParams'], ), false),
		'ajax' => 'true',
		'class' => 'block',
	)) . '
';
