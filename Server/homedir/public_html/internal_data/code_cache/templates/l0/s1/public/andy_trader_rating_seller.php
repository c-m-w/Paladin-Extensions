<?php
// FROM HASH: ba7ab2c0470633d46e2ea4e076cdcd70
return array('macros' => array(), 'code' => function($__templater, array $__vars)
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Selling history for' . ' ' . $__templater->escape($__vars['seller_name']));
	$__finalCompiled .= '
	...
	if ($__templater->isTraversable($__vars['results'])) {
		foreach ($__vars['results'] AS $__vars['result']) {
			...
			if (($__vars['xf']['visitor']['user_id'] == $__vars['result']['buyer_id']) AND (($__vars['editLimit'] < $__vars['result']['timestamp']) AND (!$__templater->method($__vars['xf']['visitor'], 'hasPermission', array('trader', 'admin', ))))) {
				$__compilerTemp2[] = array(
					'_type' => 'cell',
					'html' => '<a href="' . $__templater->fn('link', array('trader/editseller', '', array('trader_id' => $__vars['result']['trader_id'], ), ), true) . '">' . 'Edit' . '</a>',
				);
			}
			if (($__vars['xf']['visitor']['user_id'] == $__vars['result']['buyer_id']) AND (($__vars['editLimit'] >= $__vars['result']['timestamp']) AND (!$__templater->method($__vars['xf']['visitor'], 'hasPermission', array('trader', 'admin', ))))) {
				$__compilerTemp2[] = array(
