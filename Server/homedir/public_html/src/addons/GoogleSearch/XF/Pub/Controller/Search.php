...
class Search extends XFCP_Search
{
    ...
    {
        if ($this->canTryAnyways())
        {
            return $this->tryAnyways('error', is_array($error) ? implode($error) : $error, $this->GoogleSearch_keywords);
        }
    ...
    public function actionResults(ParameterBag $params)
    {
        ...
        {
            if ($return->getMessage() == $this->message(\XF::phrase('no_results_found'))->getMessage() && $this->canTryAnyways())
            {
    ...
    {
        return !empty($this->filter('keywords', 'str'));
    }

    protected function canTryAnyways()
    {
        return $this->isTryAnywaysEnabled() && $this->hasQuery();
    }
