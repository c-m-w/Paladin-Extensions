...
abstract class AbstractHandler
{
    ...
    {
        return \XF::app()->getContentTypePhrase($this->getContentType(), true);
    }
    ...
    public function updateRecentCacheForUserChange($oldUserId, $newUserId)
    {
        ...
        // note that xf_th_reacted_content must already be updated
        ...
        \XF::db()->query("
            UPDATE (
                SELECT content_id FROM xf_th_reacted_content
                WHERE content_type = ?
                AND react_user_id = ?
            ) AS temp
    ...
    {
        return ($this->getGlobalStatus() && $this->getAddOnStatus() ? true : false);
    }

    public function getGlobalStatus()
    {
        $reactHandlers = \XF::app()->options()->th_reactHandlers_reactions;
        if (!array_key_exists($this->getContentType(), $reactHandlers)) {
            return true;
        }

        return $reactHandlers[$this->getContentType()];
    }

    public function getAddOnStatus()
    {
        if (!$addOnId = $this->getAddOnId()) {
            return true;
        }

        $registry = \XF::app()->registry();
        if (!empty($registry['addOns'][$addOnId])) {
            return true;
        }
    ...
    public function getReactionBarPositions()
    {
        return false;
    }
    ...
    public function getStatHandlerKey()
    {
        return $this->statHandlerKey;
    }
}