...
class ReactedContent extends Repository
{
    ...
    public function sortReactsByType($reacts, $showAll = false)
    {
        ...
        $sortedReacts = $this->buildReactsStructure();
        ...
        return array_filter($sortedReacts);
    }
    ...
    public function buildReactsStructure()
    {
        $reacts = ['all' => []];
        foreach (array_keys(\XF::app()->container('reactionTypes')) as $reactionType) {
            $reacts[$reactionType] = [];
        }
