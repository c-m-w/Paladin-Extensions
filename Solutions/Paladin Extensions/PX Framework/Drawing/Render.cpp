...
namespace PX
{
    namespace Render
    {
        void SetWindowSize( unsigned uWidth, unsigned uHeight )
        {
            uWindowWidth = uWidth;
        ...
        void InitializeDirectX( )
        {
            ...
            pDevice->GetCreationParameters( new D3DDEVICE_CREATION_PARAMETERS );
        }
        ...
        void SetCursor( ECursor curType )
        {
            SetCursor( hCursors[ curType ] ? hCursors[ curType ] : hCursors[ NONE ] );
        }

        void SetWindowProc( IDirect3DDevice9* pTargetDevice )
        {
