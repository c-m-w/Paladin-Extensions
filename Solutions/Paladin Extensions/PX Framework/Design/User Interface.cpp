		...
		bool PX_API HandleWindowInput( )
		{
			...
			if ( bCreatedWindow )
			{
				...
				else if ( PX_INPUT.GetKeyState( VK_LBUTTON ) )
				{
					...
					{
						bDrag = true;
						if ( !pntOldCursorPosRelative.x || !pntOldCursorPosRelative.y )
							pntOldCursorPosRelative = pntCursorPosRelative;
	...
	namespace Widgets
	{
		...
		void PX_API ColorPicker( )
		{
			...
			if ( bNewColor && pActiveEditColor )
			{
				uCurrentSequence = 0u;
				clrChosenColor = {
					pActiveEditColor->GetColor( uCurrentSequence ).rfl,
		...
		int PX_API IncrementalCombobox( unsigned uButtonHeight, Types::cstr_t szTitle, const std::deque< Types::cstr_t > &dqOptions, int &iIncrement, int iMax, unsigned uSelectedOption )
		{
			...
			const auto pOutput = nk_window_get_canvas( pContext );
			if ( nk_combo_begin_label( pContext, szTitle, nk_vec2( recComboboxBounds.w, float( uButtonHeight ) * ( dqOptions.size( ) + 1 ) + 4 * ( dqOptions.size( ) ) ) ) )
			{
				bDrawComboboxArrow = true;
				recComboboxWindowBounds = pContext->current->bounds;
				nk_layout_space_begin( pContext, NK_STATIC, uButtonHeight, dqOptions.size( ) + 2 );
				nk_layout_space_push( pContext, nk_rect( 1.f, 1.f, recComboboxBounds.w / 2.f - 1.5f, uButtonHeight ) );
				iCurrentRowUsedColumns -= 2;
				if ( Button( EPosition::LEFT, PX_XOR( "+" ), false, false ) && iIncrement < iMax )
					iIncrement++;
				nk_layout_space_push( pContext, nk_rect( recComboboxBounds.w / 2.f + 1.f, 1.f, recComboboxBounds.w / 2.f - 1.5f, uButtonHeight ) );
				if ( Button( EPosition::RIGHT, PX_XOR( "-" ), false, false ) && iIncrement > 0 )
				{
					iIncrement--;
					uSelectedOption = 0;
				}
				auto y = uButtonHeight + 1.f;
				for ( unsigned i { }; i < dqOptions.size( ); i++ )
				{
					if ( !dqOptions.at( i ) )
					{
		...
		{
			const auto fnGetTrueIndex = [ ]( const std::deque< colored_text_t > *pTabs, unsigned uCurrentTab, unsigned uIndex )
			{
				auto uReturn = 0u;
				for ( auto u = 0u; u < uCurrentTab; u++ )
					uReturn += pTabs[ u ].size( );
			...
			if ( nk_combo_begin_label( pContext, szTitle, nk_vec2( recComboboxBounds.w, float( uButtonHeight ) * ( dqCurrentItems.size( ) + 1u ) + 4 * dqCurrentItems.size( ) ) ) )
			{
				...
				for ( unsigned i { }; i < dqCurrentItems.size( ); i++ )
				{
					...
					nk_layout_space_push( pContext, nk_rect( i == 0 ? 0.f : 5.f, y, recComboboxWindowBounds.w, uButtonHeight ) );
					pContext->style.contextual_button.text_active = pContext->style.contextual_button.text_hover = pContext->style.contextual_button.text_normal = dqCurrentItems[ i ].clrText;
					if ( nk_combo_item_label( pContext, dqCurrentItems.at( i ).szText, NK_TEXT_LEFT ) )
					{
						uSelectedOption = fnGetTrueIndex( pItems, uTab, i );
						bReturn = true;
					}
		...
		int PX_API Slider( cstr_t szTitle, char *szInputBuffer, int iMin, int iMax, int iCurrentValue, unsigned uStartX, unsigned uStartY, unsigned uWidth, unsigned uHeight, bool bIgnorePopup /*= false*/ )
		{
			...
			if ( uSliderIntCounter >= vecSliderInfo.size( ) )
				vecSliderInfo.emplace_back( slider_info_t( ) );
			auto &siCurrent = vecSliderInfo.at( uSliderIntCounter );
		...
		void PX_API Graph( float x, float y, float w, float h, int iVerticalGridlines, int iHorizontalGridlines, const struct nk_vec2 *vecPoints, std::size_t zPoints, struct nk_vec2 *vecDots, std::size_t zDots, int &iDraggingDot )
		{
			...
			for ( auto z = 0u; z < zDots; z++ )
			{
				...
				const auto recDot = nk_rect( vecPoint.x - 2.5f, vecPoint.y - 2.5f, 4.f, 4.f );
				nk_fill_circle( pOutput, recDot, clrBlue );
				if ( nk_input_is_mouse_hovering_rect( &pContext->input, recDot )
					|| iDraggingDot == z )
				{
					SetWidgetActive( ECursor::CURSOR_HAND );
					if ( !bHoveringDot
						&& PX_INPUT.GetKeyState( VK_LBUTTON ) == true )
					{
						bHoveringDot = true;
						vecDots[ z ] = pContext->input.mouse.pos;
						vecDots[ z ].x -= recLocation.x;
						vecDots[ z ].y = recLocation.h - recLocation.h * ( ( vecDots[ z ].y - recLocation.y ) / recLocation.h );
			...
			for ( auto z = 0u; z < zPoints - 1; z++ )
			{
				...
				if ( vecCurrent[ 0 ].y < recLocation.y )
					vecCurrent[ 0 ].y = recLocation.y;
				if ( vecCurrent[ 0 ].y > recLocation.y + recLocation.h )
					vecCurrent[ 0 ].y = recLocation.y + recLocation.h;
				if ( vecCurrent[ 0 ].x < recLocation.x )
					vecCurrent[ 0 ].x = recLocation.x;
				if ( vecCurrent[ 0 ].x > recLocation.x + recLocation.w )
					vecCurrent[ 0 ].x = recLocation.x + recLocation.w;
				...
				if ( vecCurrent[ 1 ].y < recLocation.y )
					vecCurrent[ 1 ].y = recLocation.y;
				if ( vecCurrent[ 1 ].y > recLocation.y + recLocation.h )
					vecCurrent[ 1 ].y = recLocation.y + recLocation.h;
				if ( vecCurrent[ 1 ].x < recLocation.x )
					vecCurrent[ 1 ].x = recLocation.x;
				if ( vecCurrent[ 1 ].x > recLocation.x + recLocation.w )
					vecCurrent[ 1 ].x = recLocation.x + recLocation.w;
