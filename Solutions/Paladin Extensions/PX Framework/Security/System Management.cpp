...
namespace PX::sys
{
	...
	/// This function has been converted to shellcode below, so it will function properly in debug mode. It should not be deleted in case we need it.
	...
	PX_DEF CALL_DLL_THREAD_SIZE = 3765;
	...
	bool PX_API DLLManualMap( const LPVOID& pDLL, const std::wstring& wstrExecutableName, injection_info_t* injInfo )
	{
		...
		if ( !dbg::Assert( WriteProcessMemory( hTarget, pMemory, injInfo, sizeof( injection_info_t ), nullptr ) ) )
		{
			fnCleanup( true );
			return false;
		}

		if ( !dbg::Assert( WriteProcessMemory( hTarget, PVOID( static_cast< injection_info_t* >( pMemory ) + 1 ), bCallDLLThread, CALL_DLL_THREAD_SIZE, nullptr ) ) )
		{
			fnCleanup( true );
			return false;
		}
		...
		DWORD dwExitCode { };
		if ( !dbg::Assert( hThread = CreateRemoteThread( hTarget, nullptr, 0, LPTHREAD_START_ROUTINE( static_cast< injection_info_t* >( pMemory ) + 1 ), pMemory, 0, nullptr ) ) ||
