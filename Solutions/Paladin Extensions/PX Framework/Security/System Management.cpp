...
namespace PX::sys
{
	...
	bool PX_API DLLManualMap( const LPVOID& pDLL, const std::wstring& wstrExecutableName, Types::injection_info_t* injInfo )
	{
		...
		if ( !dbg::Assert( WriteProcessMemory( hTarget, PVOID( static_cast< Types::injection_info_t* >( pMemory ) + 1 ), bCallDLLThread, CALL_DLL_THREAD_SIZE, nullptr ) ) )
		{
			fnCleanup( true );
			return false;
		}
		...
		fnWipeMemory( PVOID( static_cast< Types::injection_info_t* >( pMemory ) + 1 ), CALL_DLL_THREAD_SIZE );
