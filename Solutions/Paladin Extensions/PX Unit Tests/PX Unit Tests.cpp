...
#define PX_ASSERT_INFO new __LineInfo( L"PX Unit Tests.cpp", __func__, __LINE__ )
...
#undef PX_WSTR
			...
			namespace Security
			{
				...
				{
				public:
					// todo: @cole GeneratePostData, InitializeConnection, CleanupConnection, Request, RequestExtension, Login, RequestExtensionInformation
				};

				namespace AnalysisProtection
				{
					TEST_CLASS( DebuggerPrevention )
					{
					public:
					};

					TEST_CLASS( DebuggerDetection )
					{
					public:
					};

					TEST_CLASS( AnalysisSoftwareDetection )
					{
					public:
					};

					TEST_CLASS( DumpPrevention )
					{
					public:
					};
