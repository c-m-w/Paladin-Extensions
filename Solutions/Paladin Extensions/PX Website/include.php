<?php
	...
	{
		return openssl_encrypt( $plain_text, EncryptionMethod, 
			isset( $_SESSION[ "key" ] )
				? $_SESSION[ "key" ]
				: Keys[ "key" ], 0,
			isset( $_SESSION[ "iv" ] )
				? $_SESSION[ "iv" ]
				: Keys[ "iv" ] ); // Base64 encode then encrypt.
	}
	...
	function Decrypt( $cipher_text )
	{
		...
		return openssl_decrypt( str_replace( " ", "+", $cipher_text ) . PHP_EOL, EncryptionMethod,
			isset( $_SESSION[ "key" ] )
				? $_SESSION[ "key" ]
				: Keys[ "key" ], 0,
			isset( $_SESSION[ "iv" ] )
				? $_SESSION[ "iv" ]
				: Keys[ "iv" ] ); // Base64 decode then decrypt.
	}
	...
	define( "UnsafeKeywords", array(
		" drop ", " delete ", " select ", " alter ", " table ", " class ",
		" DROP ", " DELETE ", " SELECT ", " ALTER ", " TABLE ", " CLASS " ) ); // TODO: Add all unsafe keywords.
	...
	function PutLoginAttempt( $user_id, $cur_unique_id, $hardware, $code, $secondary_groups = "NONE" )
	{
		...
		if ( $hardware == NULL )
		 ...
		 {
			$result = $sql_connection->query( 'SELECT unique_id FROM px_unique_id WHERE user_id = ' . $user_id
		...
		$sql_connection->query( 'INSERT INTO px_logins VALUES ('
			. ( int )$user_id . ', '
			. ( int )time( ) . ', '
			. ( int )$cur_unique_id . ', '
			. $unique_id . ', "'
	...
	function ValidateHardware( $unique_id, $user_id, $hardware )
	{
		...
		$result = $sql_connection->query( 'SELECT * FROM px_unique_id WHERE user_id = ' . $user_id
			. ' AND cpu = "' . $hardware[ "cpu" ]
			. '" AND gpu = "' . $hardware[ "gpu" ]
			. '" AND display = "' . $hardware[ "display" ]
			. '" AND os = "' . $hardware[ "os" ]
	...
	function GetExtensionInformation( )
	{
		...
		while( ( $row = $result->fetch_assoc( ) ) != NULL )
		{
			$info = array(
				...
				"Estimated Next Update" => ( string )$row[ "estimated_next_update" ],
				"Last Update" => ( string )$row[ "last_update" ],
