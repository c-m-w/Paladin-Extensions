...
public:
	virtual bool RegisterConCommandBase( ConCommandBase *pVar )
	{
		...
		pConVar->RegisterConCommand( pVar );
		return true;
	}
};
...
{
	if ( !pConVar || s_bRegistered )
		return;

	assert( s_nDLLIdentifier < 0 );
	s_bRegistered = true;
	s_nCVarFlag = nCVarFlag;
	s_nDLLIdentifier = pConVar->AllocateDLLIdentifier( );
...
void ConVar_Unregister( )
{
	...
	assert( s_nDLLIdentifier >= 0 );
	pConVar->UnregisterConCommands( s_nDLLIdentifier );
	s_nDLLIdentifier = -1;
	s_bRegistered = false;
}
...
void ConCommandBase::Shutdown( )
{
	if ( pConVar )
	{
		pConVar->UnregisterConCommand( this );
	}
}
