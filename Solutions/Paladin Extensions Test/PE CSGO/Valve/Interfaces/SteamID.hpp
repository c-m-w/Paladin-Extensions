...
public:
	...
	virtual EGCResult SendMessage( DWORD unMsgType, const void *pubData, DWORD cubData ) = 0;
	virtual bool IsMessageAvailable( DWORD *pcubMsgSize ) = 0;
	virtual EGCResult RetrieveMessage( DWORD *punMsgType, void *pubDest, DWORD cubDest, DWORD *pcubMsgSize ) = 0;
};
...
public:
	...
	//-----------------------------------------------------------------------------
	CSteamID( DWORD unAccountID, EUniverse eUniverse, EAccountType eAccountType )
	{
	...
	//			eAccountType -	Type of account
	//-----------------------------------------------------------------------------
	CSteamID( DWORD unAccountID, unsigned int unAccountInstance, EUniverse eUniverse, EAccountType eAccountType )
	{
	...
	//			eAccountType -	Type of account
	//-----------------------------------------------------------------------------
	void Set( DWORD unAccountID, EUniverse eUniverse, EAccountType eAccountType )
	{
	...
	//			eAccountType -	Type of account
	//-----------------------------------------------------------------------------
	void InstancedSet( DWORD unAccountID, DWORD unInstance, EUniverse eUniverse, EAccountType eAccountType )
	{
		m_steamid.m_comp.m_unAccountID = unAccountID;
		m_steamid.m_comp.m_EUniverse = eUniverse;
		m_steamid.m_comp.m_EAccountType = eAccountType;
	...
	//-----------------------------------------------------------------------------
	void FullSet( __int64 ulIdentifier, EUniverse eUniverse, EAccountType eAccountType )
	{
	...
	// Input  : ulSteamID -		64-bit representation of a Steam ID
	//-----------------------------------------------------------------------------
	void SetFromUint64( __int64 ulSteamID )
	{
	...
	//-----------------------------------------------------------------------------
	__int64 GetStaticAccountKey( ) const
	{
		// note we do NOT include the account instance (which is a dynamic property) in the static account key
		return ( __int64 )( ( ( ( __int64 )m_steamid.m_comp.m_EUniverse ) << 56 ) + ( ( __int64 )m_steamid.m_comp.m_EAccountType << 52 ) + m_steamid.m_comp.m_unAccountID );
	}
	...
	// simple accessors
	void SetAccountID( DWORD unAccountID )
	{
		m_steamid.m_comp.m_unAccountID = unAccountID;
	}
	void SetAccountInstance( DWORD unInstance )
	{
		m_steamid.m_comp.m_unAccountInstance = unInstance;
	}
	...
	DWORD GetUnAccountInstance( ) const
	{
	...
	static const char * Render( __int64 ulSteamID );	// static method to render a uint64 representation of a steam ID to a string
...
#else
			DWORD				m_unAccountID : 32;			// unique account identifier
			unsigned int		m_unAccountInstance : 20;	// dynamic instance ID
			unsigned int		m_EAccountType : 4;			// type of account - can't show as EAccountType, due to signed / unsigned difference
			EUniverse			m_EUniverse : 8;	// universe this account belongs to
#endif
