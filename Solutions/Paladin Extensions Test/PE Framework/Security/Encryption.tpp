...
namespace Paladin
{
    namespace xors
    {
        ...
        template< typename _char, int... iIndex > constexpr _char CXorString< _char, SIndexList< iIndex... > >::EncryptCharacter( const _char _chCharacter, int iIndex )
        {
            return _chCharacter ^ ( _chXorKey + iIndex );
        }

        template< typename _char, int... iIndex > constexpr CXorString< _char, SIndexList< iIndex... > >::CXorString( const _char * const String ): _chValue { EncryptCharacter( String[ iIndex ], iIndex ) ... }
        { }
    }
}
